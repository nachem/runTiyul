...
| Prepared release | `v1.2.2+7` establishes the first publishable permanent Android signing baseline, CRLF-safe CI identity/build-number gates, and installed-version awareness. Local signed APK verification passed; the first tag run built both platforms but publication remained blocked by a Linux certificate-parser mismatch, now corrected for manual retry; `v1.2.1` remains unpublished |
...
| Automated validation | Format/analyze pass; 141 tests pass; permanently signed `1.2.2+7` APK builds and independently verifies with the expected package, build number, and pinned certificate; CRLF-normalized metadata, workflow/site syntax, wiki links, and diff checks pass |
...
6. Publish the permanent-key `v1.2.2` baseline, secure an independent key
