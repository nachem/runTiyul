...
| Area | Status |
| --- | --- |
| Flutter Android app | Implemented; `1.4.6+16` permanently signed, published, and independently artifact-verified on 2026-09-24, not device-installed. Earlier `1.4.0+10` was installed in place and cold-launched on a Pixel 10; Android 14 emulator verification also exists. |
| Flutter iOS app | Unsigned `1.4.6+16` IPA built and published by macOS CI; checksum and provenance verified, runtime and sideloading unverified |
...
| Non-road trail snapping | Published v1.4.6: preserve nearby arms of a single winding path/track as snap candidates and keep near-exact taps before choosing shorter alternatives. Prevents hairpin endpoint collapse and unintended jumps to nearby roads. 365 tests, analyzer, and hosted release checks pass; reported real trail/device validation remains pending. |
...
| Automated validation | On 2026-09-24, v1.4.6 passed 365 local tests, 105-file formatting, full analysis, hosted CI/CodeQL, both release builds, and independent checksum/signature/provenance/latest-link checks. No mobile install, live map-provider probe, outdoor verification, or iOS runtime test occurred. |
...
phone. The 2026-09-24 algorithm fixes are released in v1.4.6; mobile feedback
remains the next verification step.
...
phone with actual regional data. These improvements are released in v1.4.5;
short planning connectors do not establish real access or topology.
