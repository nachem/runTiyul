...
| Area | Status |
| --- | --- |
| Flutter Android app | Implemented; `1.4.1+11` permanently signed, published, and independently artifact-verified on 2026-09-22, not device-installed. Earlier `1.4.0+10` was installed in place and cold-launched on a Pixel 10; Android 14 emulator verification also exists. |
| Flutter iOS app | Unsigned `1.4.1+11` IPA built and published by macOS CI; checksum and provenance verified, runtime and sideloading unverified |
...
| Dense route editing and matching | Sparse controls preserve full route geometry; local connected Move/Delete, inserted controls, lossless mode switching, and whole-edit Undo. Matching rejects partial/distant routes, separates nearby/grade-separated ways, honors available access tags, and ignores stale results. Full-coordinate storage and GPX metadata preservation remain; display-vector topology omissions are documented. |
...
| Map controls/source modes | Zoom, fit/reset, GPS recenter, saved-trail visibility, source/base-layer controls, safe-area panels, and content auto-fit are implemented. An unreleased MAP-008 fix stops delayed startup GPS from resetting zoom after toolbar zoom/recenter; the race remains in published v1.4.1. Untouched startup still uses neighborhood zoom. Offline preview restores the prior source; offline camera range matches online through z19. |
...
| Next release | [v1.4.2+12](releases/v1.4.2.md) preparation in progress for the delayed-startup recenter regression. Unrelated local download-screen/test edits are excluded; committed-source CI and signed artifact verification are pending. |
| Latest published release | [v1.4.1+11](releases/v1.4.1.md) published 2026-09-22 from `fcf721f` on `main`, after hosted CI and CodeQL passed. Public APK/IPA checksums, Android identity/certificate, tagged-workflow provenance, and stable latest URLs verify. Device testing remains pending. |
| Release workflow | [Run 35740728459](https://github.com/nachem/runTiyul/actions/runs/35740728459) passed metadata, Android permanent-signature/identity verification, unsigned iOS build, checksums, APK+IPA provenance, and publication for `v1.4.1`; `v1.2.1` remains an unpublished tag with no artifacts |
| Android update compatibility | `v1.4.1` retains the `v1.2.2` permanent certificate and increases `versionCode` to 11. On 2026-09-04, a locally rebuilt, matching-signed `1.4.0+10` APK replaced the installed `1.4.0+10` app on a Pixel 10 with `pm install -r`; Android preserved the original install timestamp and recorded a new update timestamp. A true cross-version upgrade and manual inspection of retained routes/maps remain unverified. Published builds through `v1.2.0` used incompatible ephemeral debug keys and require a one-time uninstall |
...
| Automated validation | On 2026-09-22, the unreleased recenter follow-up passed 13 focused widget tests; the full VS Code runner reported 305 passes and focused analysis passed. Full analysis reports an unrelated warning in the concurrently edited offline-download screen, left intact. The earlier v1.4.1 hosted CI/build/publication and artifact checks remain valid for that tag. No device test or new APK/release was performed for this follow-up. |
...
Camera regression gate: publish the tested MAP-008 delayed-startup correction
in a new version, then verify toolbar zoom/recenter with slow GPS on a phone.
Do not describe the existing v1.4.1 APK as containing that correction.
