...
| Area | Status |
| --- | --- |
| Flutter Android app | Implemented; `1.4.2+12` permanently signed, published, and independently artifact-verified on 2026-09-22, not device-installed. Earlier `1.4.0+10` was installed in place and cold-launched on a Pixel 10; Android 14 emulator verification also exists. |
| Flutter iOS app | Unsigned `1.4.2+12` IPA built and published by macOS CI; checksum and provenance verified, runtime and sideloading unverified |
...
| Map controls/source modes | Zoom, fit/reset, GPS recenter, saved-trail visibility, source/base-layer controls, safe-area panels, and content auto-fit are implemented. Published v1.4.2 fixes MAP-008 delayed startup GPS resetting zoom after toolbar zoom/recenter; both GPS completion orders are regression-tested. Untouched startup still uses neighborhood zoom. Offline preview restores the prior source; offline camera range matches online through z19. |
...
| Next release | [v1.4.3+13](releases/v1.4.3.md) preparation: Center view preserves zoom and orientation, with separate explicit Fit. All 28 focused and 312 full-suite tests pass; 101-file formatting and analysis are clean after removing an empty test file and unused local from prior committed changes. Corrected-source hosted CI and public artifact checks are pending. |
...
| Automated validation | On 2026-09-22, 13 focused camera tests passed and v1.4.2 committed-source hosted CI passed full formatting, clean analysis, and tests. CodeQL, Android/iOS release builds, publication, public checksums, identity/signature, provenance, and latest links passed. The unrelated local download-screen warning/test deletion is excluded from the released snapshot. No physical-device test was performed. |
