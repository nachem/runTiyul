...
| Area | Status |
| --- | --- |
| Flutter Android app | Implemented; `1.4.4+14` permanently signed, published, and independently artifact-verified on 2026-09-23, not device-installed. Earlier `1.4.0+10` was installed in place and cold-launched on a Pixel 10; Android 14 emulator verification also exists. |
| Flutter iOS app | Unsigned `1.4.4+14` IPA built and published by macOS CI; checksum and provenance verified, runtime and sideloading unverified |
...
| Checkpoint planning (workspace) | Unpublished improvement: live routed preview, 16 nearby candidates within 150 m, long-bend pathfinding, bounded corridor expansion, and explicit short approach/gap connections. Raw checkpoint taps survive draft edits/Undo; Save retains the preview. GPX matching and strict navigation remain separate. 348 tests, analysis, and debug build pass; no mobile/field validation. |
...
| Latest published release | [v1.4.4+14](releases/v1.4.4.md) published 2026-09-23 from `23d2f40` on `main`, after hosted CI and CodeQL passed. Public APK/IPA checksums, Android identity/certificate, exact tagged-workflow provenance, and stable latest URLs verify. Device testing remains pending. |
| Next release | [v1.4.5+15](releases/v1.4.5.md) is being prepared on 2026-09-23 to publish the checkpoint-routing improvements. Fresh validation passed 348 tests, 104-file formatting, full analysis, and metadata checks; hosted checks, tagging, and public artifact verification remain pending. |
| Release workflow | [Run 35840664878](https://github.com/nachem/runTiyul/actions/runs/35840664878) passed metadata, Android permanent-signature/identity verification, unsigned iOS build, checksums, APK+IPA provenance, and publication for `v1.4.4`; `v1.2.1` remains an unpublished tag with no artifacts |
| Android update compatibility | `v1.4.4` retains the `v1.2.2` permanent certificate and increases `versionCode` to 14. On 2026-09-04, a locally rebuilt, matching-signed `1.4.0+10` APK replaced the installed `1.4.0+10` app on a Pixel 10 with `pm install -r`; Android preserved the original install timestamp and recorded a new update timestamp. A true cross-version upgrade and manual inspection of retained routes/maps remain unverified. Published builds through `v1.2.0` used incompatible ephemeral debug keys and require a one-time uninstall |
...
| Automated validation | On 2026-09-23, the unpublished checkpoint update passed all 348 tests, formatting of changed Dart files, full analysis, and Android debug build. Only desktop/web targets were connected. Published v1.4.4 separately passed hosted CI/CodeQL, both platform builds, and artifact verification. No live map-provider probe, device install/field test, or iOS runtime validation occurred for the checkpoint update. |
...
phone with actual regional data. These workspace improvements are not yet
released; short planning connectors do not establish real access or topology.
