...
### 2026-09-24 non-road trail snapping (released in v1.4.6)
...
unchanged. [v1.4.6+16](releases/v1.4.6.md) was published on 2026-09-24.
Tagged commit `7816c7673096ac60634958737ce86a9a3ea33041` passed CI `36030535334`
and CodeQL `36030534648` before tagging. [Release run
36030762592](https://github.com/nachem/runTiyul/actions/runs/36030762592) passed
signed Android build/identity, unsigned iOS packaging, checksums, provenance,
and publication. Independent public APK/IPA downloads match the manifest and
GitHub digests. Android package/version/permanent certificate, both exact
tagged-source attestations, and stable latest URLs verify. Hashes are recorded
in the release notes. Phone testing is planned by the user but not yet verified.

### 2026-09-23 checkpoint route selection (released in v1.4.5)

- **RTE-003/RTE-011:** regressions reproduced a valid multi-feature hairpin
  rejected by chord-based length limits and a connected way hidden by six
  nearer fragments. Checkpoint planning is now distinct from recorded GPX
  matching: it considers every checkpoint, up to 16 nearby way candidates
  within 150 m, and bounded mapped paths without the GPX chord/corridor test.
  Candidate routing searches all next targets together rather than separately
  for every pair. Fully mapped sequences take priority; route length and snap
  distance choose between viable candidates. The path budget is the larger of
  10 km or eight times the checkpoint separation, capped at 100 km per leg.
- Points within 40 m snap; points farther away remain at their selected
  positions. A nearby way within 150 m can serve such a checkpoint through
  short, explicitly unmapped approach/departure segments. Distinct nearby
  checkpoints cannot collapse onto the same projected location merely to avoid
  a loop. Stops less than the GPX 20 m sampling interval are retained.
- Only when a target cannot be reached through mapped edges, planning may use
  a connector of at most 12 m between a dangling endpoint and a same-level
  routable way in its forward continuation (60-degree cone, checked from both
  sides when both are dangling). A 50 m spatial index bounds gap candidates;
  multiple projections along one segment are connected in order. Gap distance
  carries a 25-times cost and only those segments are labeled unmapped. These
  links are separate from strict graph adjacency and never enable recovery
  across a gap, bridge, or prohibited connector. No walkability is inferred.
- `networkForCheckpoints` loads the route corridor and, if whole legs remain
  unrouted, tries a second tile ring within the existing 256-tile limit. Cached
  tiles are reused; Offline never opens an HTTP source. Empty/missing coverage
  leaves a direct plan; unavailable source or data-budget limits remain explicit.
- **RTE-009/RTE-012:** new Checkpoints routes show their routed preview while
  selecting points instead of waiting for Save. Original taps are held separately
  from generated geometry, preserved through control insertion and Undo; further
  checkpoint edits re-evaluate the sequence. Save sends the exact preview and
  does not trigger a second background snap. Existing dense-route editing remains
  lossless and local; explicit Snap on saved manual routes uses the checkpoint
  planner, while GPX keeps its prior matching mode and metadata protection.

Validation on 2026-09-23: all **348** tests passed in the full VS Code Flutter
runner; `flutter analyze --no-pub` passed; changed Dart files were formatted;
`flutter build apk --debug --no-pub` succeeded. The existing `flutter_tts` Kotlin
warning is non-fatal. Tests cover long/close hairpins, candidate crowding,
shortest connected alternatives, approach segments, collinear/T-junction and
multiple short gaps, parallel/bridge separation, 1,500 unrelated fragments,
distinct close stops, bounded corridor expansion, Offline isolation, actual
checkpoint preview/Undo/Save, saved manual routing, and GPX behavior. `flutter
devices` listed Windows, Chrome, and Edge only; no mobile target was connected.

Limits: these are synthetic and mocked scenarios, not a claim that all real
routes connect. Actual topology, access, barriers, tile coverage, and long-route
phone performance still need device/data verification. User-planned approach
and gap connections are explicitly unmapped and can be unsuitable in reality.
Raw checkpoints and per-segment diagnostics are draft-only; saved route geometry
persists, but a reopened route does not recover those raw taps. No device install,
live map-provider probe, signed release, version bump, commit, or publication
was performed during the initial implementation checks. On 2026-09-23,
publication was requested: [v1.4.5+15](releases/v1.4.5.md) was prepared with
348 passing candidate tests, a clean 104-file formatting check, full analysis,
and matching release metadata. Commit
`da262d86d62969df3373688f178710a6a2bc7b00` passed hosted CI `35884094142` and
CodeQL `35884093234` before the immutable `v1.4.5` tag was pushed. [Release run
35884381841](https://github.com/nachem/runTiyul/actions/runs/35884381841) passed
signed Android build/identity, unsigned iOS packaging, checksums, provenance,
and publication. Independent public APK/IPA downloads match the checksum
manifest and GitHub digests. Android package/version/permanent certificate,
both exact tagged-source attestations, and stable latest URLs verify. Artifact
hashes are recorded in the release notes. No device installation or field test
occurred; the earlier v1.4.4 tag and artifacts remain unchanged.
...
- **RTE-003/RTE-011:** failing regressions reproduced a sparse right-angle
  ...
  endpoint missing from the parent segment's vertices. Sparse observation gaps
  over 80 m now allow a reasonable mapped bend; observed points and dense spans
  retain corridor checks and the existing whole-match detour bound.
- `TrailRouter` splits segments at same-level feature endpoints within its
  0.01 m coordinate precision, using a latitude-sorted endpoint index and bounds
  filtering. Snapped anchors attach to the surrounding split segment. Parallel
  ways, bridge/ground crossings, and restricted connectors stay separate.
  Arbitrary interior/interior crossings are not inferred as junctions.
- `planWaypoints` compares up to six nearby candidates, prioritizing connected
  legs before distance/shape cost. In planning, points within 40 m snap, while
  distant points retain their coordinates. If no reasonable mapped connection
  exists, the plan keeps an explicit direct segment instead of rejecting the
  edit. Strict matching and navigation-recovery APIs still reject such gaps.
- **RTE-004/RTE-009/RTE-012:** the editor reports direct unmapped segments;
  append/move/delete affect only the local span, duplicate snapped taps do not
  add controls, and Undo restores geometry plus diagnostics. Whole-route
  snapping preserves original intermediate geometry across gaps and reports
  partial alignment. Off-map GPX stops retain matching elevation/timestamps
  through SQLite save/reload; stale results still cannot overwrite edits.
- **OFF-012:** current-map chips remain tappable but locked sources invoke the
  eligibility/confirmation flow instead of silently selecting an unusable
  format. Authorized Esri shows `Satellite · DEV` after seven taps and explicit
  confirmation. The dedicated VS Code **Running App (authorized raster DEV)**
  launch profile compiles the existing authorized-build flag; ordinary build
  defaults are unchanged. Esri retains attribution, the development pacing
  policy, and its provider/format namespace. Provider permission was confirmed
  by the user for development use, not independently audited.

Validation: all **331** tests passed in the full VS Code Flutter runner;
`flutter analyze --no-pub` passed, and changed Dart files were formatted.
`flutter build apk --debug --no-pub
--dart-define=ALLOW_AUTHORIZED_VIEW_RASTER_DEV_DOWNLOADS=true` built successfully
after the version bump to `1.4.4+14`. The existing `flutter_tts` Kotlin warning
remains non-fatal. Automated scenarios cover bends, hairpins, T-junctions in
...
The corrected snapshot `f32b565` passed hosted CI and CodeQL before tagging.
[v1.4.3+13](releases/v1.4.3.md) is published with both platform builds and
independent public checksum, Android identity/signature, provenance, and
latest-link verification complete. No physical-device validation occurred.
...
| Offline area selection | Implemented; analyzer/unit/widget-tested | Two corners, zoom, tile/storage/time estimates, adjustable caps, and confirmation. **MBTiles / vector** and **Current map: _layer_** remain visible. The tappable current-map choice routes locked selections through eligibility checks and seven-tap confirmation; it does not bypass provider authorization. Esri/Topographic need the separate authorized DEV profile/flag. Confirmed sources display `DEV`, persist the unlock on that device, and retain provider/format identity for resume/render/delete. |
...
Latest local code validation, hosted `v1.4.6` release validation, and independent
public artifact checks completed on 2026-09-24.
Earlier dated entries below remain historical evidence:

| Command | Result |
| --- | --- |
| Public `v1.4.6+16` non-road trail release (2026-09-24) | All 365 candidate tests, 105-file formatting, and full analysis passed. Tagged commit `7816c76` passed CI `36030535334` and CodeQL `36030534648`. [Release run 36030762592](https://github.com/nachem/runTiyul/actions/runs/36030762592) passed signed Android identity/build, unsigned iOS packaging, checksums, provenance, and publication. Public APK (62,905,152 bytes) and IPA (16,060,156 bytes) downloads independently match the manifest and GitHub digests. Android SDK inspection verifies the permanent certificate and `1.4.6+16`; both attestations verify the exact tagged workflow/source commit on GitHub-hosted runners; stable latest URLs return HTTP 200 with matching sizes. Hashes are in the [release notes](releases/v1.4.6.md). Mobile installation and field validation remain pending. |
...
| GitHub Actions `Continuous integration` | Run [`35750005333`](https://github.com/nachem/runTiyul/actions/runs/35750005333) passed setup, dependency install, format, analyze, and the complete test suite for release commit `f32b565` on 2026-09-22. The dependency-review job correctly skipped on a push and still requires pull-request verification. |
| GitHub Actions CodeQL | Run [`35750004979`](https://github.com/nachem/runTiyul/actions/runs/35750004979) passed for release commit `f32b565` on 2026-09-22. |
