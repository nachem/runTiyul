...
| MAP-008 | The primary map shall provide one-tap zoom, zoom-preserving centering, and a separate explicit fit action. | Both current-location centering and Center view preserve the live zoom and orientation. Center view recenters around route/track/checkpoints/selection and the known location; with no content it leaves the camera unchanged. A separate Fit route and content control explicitly adjusts zoom to frame that content. Center and fit controls cancel pending startup centering. When opened without a selected route or area and left untouched, the map centers on the current location at a neighborhood zoom once a fix is available. |
...
| MAP-012 | The app shall offer a topographic online base map alongside CyclOSM without downloading separate elevation data while browsing. | Streets, CyclOSM, Topographic (OpenTopoMap), and Satellite remain independent choices; the selected layer persists. OpenTopoMap is credited and view-only by default, overzooms its z17 tiles through camera z19, and does not request Terrarium/elevation tiles. Provider outages do not substitute another layer or force a per-tile fallback. |
| MAP-013 | Every map shall offer an independent vector roads/trails overlay for comparison with raster coverage. | The overlay shows the configured vector transportation geometry with attribution, hides below z14, debounces viewport changes, caps tile loads, and cancels obsolete work. Trail, road, and explicitly restricted-way styling are distinct. Offline mode uses only cached/local data and reports missing coverage; it never opens a remote vector source. |
...
| RTE-003 | The user shall be able to create a route by placing, moving, undoing, and deleting map waypoints. | New Checkpoints routes with snapping enabled preview a connected route before saving, preserve the ordered checkpoint inputs, and search bounded nearby alternatives without treating straight chords as recorded tracks. Points within 40 m snap; more distant checkpoints retain their positions and can reach nearby ways using explicitly unmapped approaches. Short plausible map-fragment gaps may be used only as reported planning connections, after mapped alternatives. Direct whole-leg fallback remains available for missing or out-of-budget coverage. Follow-trails local edits retain their geometry-preserving behavior. Undo restores a complete edit, and Save retains the preview. |
| RTE-004 | Manual planning may retain direct connections between waypoints where no mapped route is available. | The editor reports direct unmapped segments and whole-route snapping reports retained direct connections. This does not establish traversability, access permission, or a navigation-recovery path; recovery remains strictly mapped. |
...
| RTE-011 | Any saved or imported route shall expose a **Snap to trails** action. | Manual routes use checkpoint pathfinding that can follow long bends and chooses among nearby connected candidates, including separate nearby arms of a single curved feature. Among equally connected plans, existing near-exact snaps within 2 m are retained before minimizing route length, without preferring road or trail class. Imported GPX routes retain shape-constrained matching rather than treating recorded samples as checkpoints. Unmatchable points and original unmapped geometry are retained and reported, never dropped. Strict topology keeps nearby parallel ways and grade-separated crossings distinct and honors available foot/access restrictions; planning-only short-gap links are never exposed to navigation recovery as mapped edges. Failed or stale results leave the saved route unchanged, preserving identity/source and matching GPX metadata. |
| RTE-012 | Dense routes shall be editable using sparse controls without discarding their full geometry. | Opening a route derives at most 32 automatic controls around important shape changes/endpoints; markers are culled by viewport and spacing. Users can select controls directly or add a control to a hidden section. Move/delete recalculates only neighboring sections, with reported direct fallback where mapped routing is unavailable. Mode switching preserves all coordinates, and Undo restores a complete edit including its direct-segment diagnostics. Unchanged sections retain optional GPX metadata on save. |
...
| NAV-005 | The app shall allow north-up and course-up presentation plus optional continuous current-position following. | The recording map exposes persistent **North up** / **Running direction** and Follow-position controls. Course-up uses a quality-filtered moving course, falls back to north-up until one exists, and keeps the selected orientation after map gestures. A one-finger pan disables position following; manual zoom and rotation preserve following and the current zoom. |
...
| NAV-007 | Off-route recovery shall prefer the shortest recognized trail/road connection that rejoins the planned route ahead within a bounded local search. | Search constrains the initial direction to within 75 degrees of a reliable moving course and rejects reversal through the starting position. The path is connected, never an open-terrain bridge; only on-route fixes advance plan progress. Recovery updates as the runner moves, uses cached/local ways offline, and renders a direction arrow and distance to rejoin. Without a suitable mapped connection or heading, the app retains the plan and reports that it is searching, without inventing a return route or claiming globally optimal/safety-certified guidance. |
...
| ACT-004 | Implausible or very inaccurate samples shall be filtered without hiding recording state. | Rejected samples do not add distance; delayed fixes use elapsed-time-aware plausibility so OS power throttling does not permanently stall the track, and a long outage resumes from a new segment without inventing distance across the gap. Diagnostics can explain filtering. |
...
| ACT-007 | The app shall clearly indicate GPS acquisition, recording, paused, and error states. | The user is never shown "recording" when the location stream has failed unnoticed; a stream failure durably pauses the activity and reports the error. |
...
| OFF-009 | Download requests shall use bounded concurrency, retry with backoff for transient failures, and honor cancellation. | The app does not launch unbounded requests or retry permanent errors forever. Current-map raster downloads use configurable provider request spacing and batch breaks, share and persist numeric or HTTP-date `Retry-After` cooldowns, and allow cancellation during waits and retries. Long cooldowns and exhausted rate-limit retries leave a resumable paused area; HTTP 401/403 pauses without retry or automatic foreground resume. Pacing never grants provider permission or guarantees against blocking. |
...
| OFF-012 | The production tile provider shall explicitly permit bulk/offline use. | Release starts with public raster downloads locked. This repository's internal developer capability may be unlocked only by seven taps plus an explicit warning/confirmation, persists on that device, and is labeled development-only. Streets/CyclOSM follow the existing public-development policy; Topographic/Satellite are included only when the special build explicitly compiles `ALLOW_AUTHORIZED_VIEW_RASTER_DEV_DOWNLOADS=true`, representing a separately granted provider permission. Neither capability makes a source production-approved. |
...
- Additional online-only display layers (for example OpenTopoMap or Esri World
  Imagery) may be offered for interactive viewing where their terms permit it.
  Ordinary builds must not bulk-download or cache them. A special internal
  development build may expose bounded downloads only when an explicit,
  separately granted permission covers that provider and the authorized build
  flag is enabled; attribution and granted limits still apply.
