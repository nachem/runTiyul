...
| RTE-011 | Any saved or imported route shall expose a **Snap to trails** action. | Matching considers connected nearby candidates while honoring the complete input geometry. It never drops unmatchable points or substitutes a distant detour. Separate nearby ways and grade-separated crossings stay disconnected; available foot/access restrictions are honored. Failed or stale matches leave the saved route unchanged, preserving identity/source and matching GPX metadata. |
| RTE-012 | Dense routes shall be editable using sparse controls without discarding their full geometry. | Opening a route derives at most 32 automatic controls around important shape changes/endpoints; markers are culled by viewport and spacing. Users can select controls directly or add a control to a hidden section. Move/delete recalculates only neighboring sections, connected mode rejects unrouteable edits, mode switching preserves all coordinates, and Undo restores a complete edit. Unchanged sections retain optional GPX metadata on save. |
