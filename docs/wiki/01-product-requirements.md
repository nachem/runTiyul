...
| MAP-008 | The primary map shall provide one-tap zoom, current-location centering, and fit/reset controls. | The user can zoom in/out, center on a fresh location fix without changing the current zoom, and fit current location plus route/checkpoint content without gestures. When opened without a selected route or area, the map centers on the current location at a neighborhood zoom once a fix is available. |
