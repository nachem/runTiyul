...
| RTE-003 | The user shall be able to create a route by placing, moving, undoing, and deleting map waypoints. | In Follow trails, nearby taps snap within 40 m and connected candidates follow mapped bends. Distant points retain their locations and connect directly when no reasonable mapped path exists. Duplicate snapped taps do not add controls. The draft can be undone or cancelled safely. |
...
| RTE-011 | Any saved or imported route shall expose a **Snap to trails** action. | Nearby observations snap to connected mapped candidates where possible. Sparse gaps may follow reasonable mapped bends; dense input geometry constrains the match. Unmatchable points and original geometry through unmapped gaps are retained as direct connections and reported, never dropped. Coincident same-level T-junction endpoints can connect to segment interiors; nearby parallel ways and grade-separated crossings remain distinct and mapped routing honors available foot/access restrictions. Failed or stale results leave the saved route unchanged, preserving identity/source and matching GPX metadata. |
